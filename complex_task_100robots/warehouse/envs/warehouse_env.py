...
class WarehouseEnv(gym.Env):
  ...
  def reset(self):
    ...
    for count in range(0,self.agent_no):
      ...
      self.robot_positions[count][2] = np.round(gauss(20, 10)) + gauss(0,0.5)
    ...
    while(count != self.tasks_no):      
      for i in range(2):
        ...
        if (num == 0): 
          self.tasks_input[count][2*i] = abs(np.round(gauss(10,5)))
          self.tasks_input[count][2*i+1] = abs(np.round(gauss(10,5)))
        elif (num == 1): 
          self.tasks_input[count][2*i] = abs(np.round(gauss(40,5)))
          self.tasks_input[count][2*i+1] = abs(np.round(gauss(40,5)))
        elif (num == 2): 
          self.tasks_input[count][2*i] = abs(np.round(gauss(70,5)))
          self.tasks_input[count][2*i+1] = abs(np.round(gauss(40,5)))
        elif (num == 3): 
          self.tasks_input[count][2*i] = abs(np.round(gauss(25,5)))
          self.tasks_input[count][2*i+1] = abs(np.round(gauss(90,5)))
        elif (num == 4): 
          self.tasks_input[count][2*i] = abs(np.round(gauss(70,5)))
          self.tasks_input[count][2*i+1] = abs(np.round(gauss(20,5)))      

      
      self.tasks_input[count][4] = ((self.robot_positions[self.min_r][0] - self.tasks_input[count][0])**2 + (self.robot_positions[self.min_r][1] - self.tasks_input[count][1])**2)**0.5 #+ gauss(0, 0.25)
      self.tasks_input[count][5] = ((self.tasks_input[count][0] - self.tasks_input[count][2])**2 + (self.tasks_input[count][1] - self.tasks_input[count][3])**2)**0.5
  ...
  def _compute_observation(self, action):
    ...
    self.simulation_time = self.simulation_time + self.robot_positions[self.min_r][2]
    ...
    # print("task before", self.tasks_input[action])
    ...
    # print("task after", self.tasks_input[action])
